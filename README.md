...
- `read_document` (safe) — extracts text from a PDF (`pdf-parse`), Word `.docx` (`mammoth`), or Excel `.xlsx` (`exceljs`) file; `bash`/`read_file` can't do this since these are binary formats, not text. Only the modern `.docx`/`.xlsx` formats are supported, not the legacy binary `.doc`/`.xls` ones — `exceljs` in particular has no `.xls` support at all. A `.pdf` with no extractable text layer (e.g. a scanned image) is reported as such rather than returned as empty text. `write_spreadsheet` (ask) — creates a `.xlsx` file from rows of structured data, mirroring `write_file`'s conventions (path resolution, `mkdir -p` of parent directories). Reading/writing `.pdf`/`.docx` was deliberately left out of scope — the user only asked for reading those, and generating them is a meaningfully bigger surface than generating a spreadsheet.
