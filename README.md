...
- **Fleet** (when `docker` is installed): `create_fleet_cell`/`list_fleet_cells`/`stop_fleet_cell`/`remove_fleet_cell` — provisions/manages isolated per-tenant containers ("cells"), each its own real Docker container with its own port and optional persistent workspace mount. Every cell is named with a fixed prefix so these tools can only ever affect containers they created themselves, never an unrelated one on the same Docker daemon.
