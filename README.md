...
- **Video**: `view_video_frames` (when `ffmpeg`/`ffprobe` are installed) — samples a handful of evenly-spaced still frames from a video file for the model to look at (not full video understanding: no motion/timing/audio); `analyze_video` (real native video+audio understanding via Gemini, needs `GEMINI_API_KEY`, independent of the primary chat provider — limited to videos under ~19MB, sent inline)
